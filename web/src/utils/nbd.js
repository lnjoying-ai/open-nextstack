...
function NBDServer(endpoint, file) {
  ...
  this._on_ws_close = function (ev) {
    this._log('close handler for state ' + this.state);
    this._log('断开连接');
    this.stop();
    if (this.onclose) {
      this.onclose();
  ...
  this._on_ws_error = function (ev) {
    this._log('error handler for state ' + this.state);
    this.stop();
    if (this.onerror) {
      this.onerror();
    }
  ...
  this._on_ws_message = function (ev) {
    ...
    for (;;) {
      ...
      if (consumed < 0) {
        this._log('handler[state=' + this.state + '] returned error ' + consumed);
        this.stop();
        break;
      }
  ...
  this._handle_cflags = function (buf) {
    ...
    this._log('client flags received: 0x' + this.client.flags.toString(16));
  ...
  this._handle_option = function (buf) {
    ...
    if (buf.byteLength < 16 + len) return 0;
  ...
  this._handle_cmd = function (buf) {
    ...
    switch (req.type) {
      ...
      case NBD_CMD_WRITE:
         ...
         * request */
        if (buf.byteLength < 28 + req.length) return 0;
        consumed += req.length;
        err = EPERM;
        break;
