    ...
    >
      <el-card class="!border-none mb-3">
        ...
        <div class="text item">
          ...
          <el-form-item :label="$t('hardware.storagePool.form.vlanId') + ':'" prop="vlanId">
            <el-input
              v-model="form.vlanId"
              class="!w-60"
              :placeholder="$t('hardware.storagePool.form.inputVlanId')"
              @input="vlanIdchange()"
            />
          </el-form-item>
...
const rules = reactive({
  ...
  vlanId: [{ required: true, message: proxy.$t('hardware.storagePool.validation.vlanIdRequired'), trigger: 'blur' }],
});
